# models.py
import enum
from datetime import datetime
from typing import List, Optional, TYPE_CHECKING

from sqlalchemy import (
    ForeignKey, String, Text, func, DateTime, Enum
)
from sqlalchemy.orm import Mapped, mapped_column, relationship

from .base import Base
from .mixins import IntIdPkMixin

if TYPE_CHECKING:
    from .product_variant import ProductVariant
    from .user import User

# --- Базовые классы и Миксины ---
# class Base(DeclarativeBase): pass
# class IntIdPkMixin:
#     id: Mapped[int] = mapped_column(primary_key=True)
class TimestampMixin:
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())


# Блок 1: Пользователи

# class User(IntIdPkMixin, TimestampMixin, Base):
#     __tablename__ = "users"
#
#     phone: Mapped[str] = mapped_column(String(20), unique=True, index=True)
#     name: Mapped[Optional[str]] = mapped_column(String(100))
#     hashed_password: Mapped[str] = mapped_column(String)
#     is_active: Mapped[bool] = mapped_column(default=True)
#
#     orders: Mapped[List["Order"]] = relationship(back_populates="user")
#     favorites: Mapped[List["Favorite"]] = relationship(back_populates="user")
#     reviews: Mapped[List["Review"]] = relationship(back_populates="user")


class OrderStatusEnum(enum.Enum):
    # PENDING_PAYMENT удален!
    AWAITING_CONFIRMATION = "Ожидает подтверждения" # Новый начальный статус
    COOKING = "Готовится"
    READY_FOR_PICKUP = "Готов к выдаче"
    COMPLETED = "Завершен" # Устанавливается после того, как клиент оплатил и забрал заказ
    CANCELLED = "Отменен"


class Order(IntIdPkMixin, TimestampMixin, Base):
    __tablename__ = "orders"

    # Связь с пользователем
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
    user: Mapped["User"] = relationship(back_populates="orders")

    # Текущий статус заказа
    status: Mapped[OrderStatusEnum] = mapped_column(
        Enum(OrderStatusEnum), default=OrderStatusEnum.AWAITING_CONFIRMATION
    )

    # "Замороженные" данные клиента на момент заказа
    customer_name: Mapped[str] = mapped_column(String(100))
    customer_phone: Mapped[str] = mapped_column(String(20))
    comment: Mapped[Optional[str]] = mapped_column(Text)

    # "Замороженная" итоговая стоимость
    total_price: Mapped[int]

    # Человеко-читаемый номер заказа (например, "A-1001"), можно генерировать в коде
    order_number: Mapped[str] = mapped_column(String(20), unique=True, index=True)

    # Содержимое заказа
    items: Mapped[List["OrderItem"]] = relationship(back_populates="order", cascade="all, delete-orphan")
    # История статусов (для отслеживания)
    status_history: Mapped[List["OrderStatusHistory"]] = relationship(back_populates="order", cascade="all, delete-orphan")
    # Платеж по заказу
    payment: Mapped["Payment"] = relationship(back_populates="order", cascade="all, delete-orphan")


class OrderItem(IntIdPkMixin, Base):
    """Строка в чеке (заказе)"""
    __tablename__ = "order_items"

    order_id: Mapped[int] = mapped_column(ForeignKey("orders.id"))
    product_variant_id: Mapped[int] = mapped_column(ForeignKey("product_variants.id"))

    order: Mapped["Order"] = relationship(back_populates="items")
    product_variant: Mapped["ProductVariant"] = relationship()

    quantity: Mapped[int]

    # "Замороженная" цена за единицу товара на момент покупки
    price_per_unit: Mapped[int]

# ===================================================================
# Блок 3: Платежи
# ===================================================================
class PaymentStatusEnum(enum.Enum):
    PENDING = "Ожидание"
    SUCCESSFUL = "Успешно"
    FAILED = "Ошибка"
    REFUNDED = "Возвращен"


class Payment(IntIdPkMixin, TimestampMixin, Base):
    __tablename__ = "payments"

    order_id: Mapped[int] = mapped_column(ForeignKey("orders.id"), unique=True)
    order: Mapped["Order"] = relationship(back_populates="payment")

    amount: Mapped[int]
    status: Mapped[PaymentStatusEnum] = mapped_column(Enum(PaymentStatusEnum), default=PaymentStatusEnum.PENDING)

    # Информация от платежной системы
    payment_system: Mapped[Optional[str]] = mapped_column(String(50)) # "Stripe", "YourPay", "Cash"
    transaction_id: Mapped[Optional[str]] = mapped_column(String(255), index=True) # ID от платежной системы

# ===================================================================
# Блок 4: История Статусов (для отладки и вебсокетов)
# ===================================================================
class OrderStatusHistory(IntIdPkMixin, TimestampMixin, Base):
    __tablename__ = "order_status_history"

    order_id: Mapped[int] = mapped_column(ForeignKey("orders.id"))
    order: Mapped["Order"] = relationship(back_populates="status_history")

    status: Mapped[OrderStatusEnum] = mapped_column(Enum(OrderStatusEnum))
    # Поле для оператора, который изменил статус (в будущем)
    # changed_by_user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id"))

# ===================================================================
# Блок 5: Пользовательский контент (Избранное, Отзывы)
# ===================================================================
class Favorite(IntIdPkMixin, TimestampMixin, Base):
    """Связь 'многие-ко-многим' для избранных товаров"""
    __tablename__ = "favorites"

    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
    product_variant_id: Mapped[int] = mapped_column(ForeignKey("product_variants.id"))

    user: Mapped["User"] = relationship(back_populates="favorites")
    product_variant: Mapped["ProductVariant"] = relationship()


class Review(IntIdPkMixin, TimestampMixin, Base):
    __tablename__ = "reviews"

    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
    order_id: Mapped[int] = mapped_column(ForeignKey("orders.id"))

    user: Mapped["User"] = relationship(back_populates="reviews")
    order: Mapped["Order"] = relationship()

    rating: Mapped[int] # Оценка от 1 до 5
    text: Mapped[Optional[str]] = mapped_column(Text)


# class OrderItem